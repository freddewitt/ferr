...
enum Commands {
    ...
    Copy {
        src:  PathBuf,
        dest: PathBuf,
        #[arg(long)] dest2: Option<PathBuf>,
        #[arg(long)] dest3: Option<PathBuf>,
        #[arg(long, value_enum, default_value = "xxhash")] hash: HashChoice,
        #[arg(long)] par2: Option<u8>,
        #[arg(long)] resume: bool,
        #[arg(long)] camera: bool,
        #[arg(long)] rename: Option<String>,
        #[arg(long)] eject: bool,
        #[arg(long)] dedup: bool,
        #[arg(long)] profile: Option<String>,
        #[arg(long)] no_preserve_meta: bool,
        #[arg(long)] no_notify: bool,
        #[arg(long)] no_pdf: bool,
    ...
    Watch {
        mount_point: PathBuf,
        #[arg(long)] dest:    Vec<PathBuf>,
        #[arg(long, value_enum, default_value = "xxhash")] hash: HashChoice,
        #[arg(long)] par2:    Option<u8>,
        #[arg(long)] camera:  bool,
        #[arg(long)] profile: Option<String>,
        #[arg(long, default_value = "3")] delay: u64,
        #[arg(long)] eject:   bool,
        #[arg(long)] quiet:   bool,
    },
    ...
    Export {
        manifest: PathBuf,
        #[arg(long, default_value = "csv")] format: ExportFormat,
...
fn run(cli: Cli) -> anyhow::Result<i32> {
    match cli.command {
        Commands::Copy { src, dest, dest2, dest3, hash, par2, resume, camera,
                         rename, eject, dedup, profile, no_preserve_meta,
                         no_notify, no_pdf, dry_run, quiet } => {
            cmd_copy(src, dest, dest2, dest3, hash, par2, resume, camera,
                     rename, eject, dedup, profile, no_preserve_meta,
                     no_notify, no_pdf, dry_run, quiet)
        }
        Commands::Verify { src_or_manifest, dest, quiet } =>
            cmd_verify(src_or_manifest, dest, quiet),
        Commands::Repair { manifest, dest } =>
            cmd_repair(manifest, dest),
        Commands::Scan { dest, manifest, since, quiet } =>
            cmd_scan(dest, manifest, since, quiet),
        Commands::Watch { mount_point, dest, hash, par2, camera,
                          profile, delay, eject, quiet } =>
            cmd_watch(mount_point, dest, hash, par2, camera, profile, delay, eject, quiet),
        Commands::Export { manifest, format, output } =>
            cmd_export(manifest, format, output),
        Commands::Report { manifest, output } =>
            cmd_report(manifest, output),
...
fn cmd_copy(
    src: PathBuf, dest: PathBuf,
    dest2: Option<PathBuf>, dest3: Option<PathBuf>,
    hash: HashChoice, par2: Option<u8>, resume: bool,
    camera: bool, rename: Option<String>, eject: bool, dedup: bool,
    profile: Option<String>, no_preserve_meta: bool,
    no_notify: bool, no_pdf: bool, dry_run_flag: bool, quiet: bool,
) -> anyhow::Result<i32> {
    let mut destinations = vec![dest];
    if let Some(d) = dest2 { destinations.push(d); }
    if let Some(d) = dest3 { destinations.push(d); }
    let (hash_algo, _hash_algo_str) = hash_choice_to_algo(&hash);

    // Charger le profil si fourni
    let (destinations, hash_algo, par2, camera, eject, rename) =
        if let Some(profile_name) = profile {
            match ferr_core::load_profile(&profile_name) {
                Ok(p) => {
                    ...
                    (p.destinations, ha, p.par2_redundancy, p.camera_mode, p.auto_eject, rename)
                }
                Err(e) => {
    ...
    if dry_run_flag {
        ...
        if !quiet {
            println!("{}", style("Mode dry-run — aucun fichier écrit").yellow().bold());
    ...
    let on_progress = {
        ...
        move |p: ferr_core::CopyProgress| {
            ...
            match p.phase {
                ...
                _ => {
                    let name = p.current_file.file_name()
    ...
    if !quiet {
        ...
        for entry in &manifest.files {
            ...
            println!("  {} {}  [{}] {}", icon, entry.path,
                     style(&entry.hash_algo).dim(), style(hash_preview).dim());
        }
        println!();
...
fn cmd_verify(src_or_manifest: PathBuf, dest: PathBuf, quiet: bool) -> anyhow::Result<i32> {
    ...
    let report = if src_or_manifest.extension()
    ...
    if !quiet {
        ...
        for p in &report.missing   { println!("  {} {}", style("MANQUANT").yellow(), p.display()); }
        for p in &report.corrupted { println!("  {} {}", style("CORROMPU").red(),    p.display()); }
        if report.exit_code() == 0 { println!("  {}", style("Tout OK ✓").green().bold()); }
    }
...
fn cmd_repair(manifest: PathBuf, dest: PathBuf) -> anyhow::Result<i32> {
    ...
    let result = ferr_par2::repair(&manifest, &dest, |pct| {
        if let Some(b) = &bar { b.set_message(format!("PAR2 : {pct}%…")); }
    });
    if let Some(b) = &bar { b.finish_and_clear(); }
...
fn cmd_scan(
    ...
    let manifest_path = manifest_path
        .unwrap_or_else(|| dest.join("ferr-manifest.json"));

    let manifest = ferr_report::load_manifest(&manifest_path)?;
    let hasher: Box<dyn ferr_hash::Hasher> = Box::new(ferr_hash::XxHasher);

    let since_dt = since.as_deref()
    ...
    let report = ferr_verify::scan_bitrot(
        &dest,
        &manifest,
        hasher.as_ref(),
        since_dt,
        |p| {
            if let Some(b) = &bar {
                b.set_message(format!(
                    "[{}/{}] {}",
                    p.scanned, p.total, p.current.display()
                ));
            }
        },
    )?;
    ...
    if !quiet {
        ...
        for entry in &report.corrupted {
            ...
            println!(
                "     attendu : {}",
                style(&entry.expected_hash).dim()
            );
            println!(
                "     actuel  : {}",
                style(&entry.actual_hash).red()
            );
        }
...
fn cmd_watch(
    ...
    let (destinations, hash_str, par2, camera, eject) = if let Some(name) = profile {
        match ferr_core::load_profile(&name) {
            Ok(p) => (p.destinations, p.hash_algo, p.par2_redundancy, p.camera_mode, p.auto_eject),
            Err(e) => {
                eprintln!("Profil non trouvé : {e}");
...
fn cmd_export(manifest_path: PathBuf, format: ExportFormat, output: PathBuf) -> anyhow::Result<i32> {
    let manifest = ferr_report::load_manifest(&manifest_path)?;
    match format {
...
fn cmd_profile(action: ProfileAction) -> anyhow::Result<i32> {
    match action {
        ProfileAction::Save { name, dest, hash, par2, camera, eject } => {
            ...
            if profiles.is_empty() {
                ...
                for p in &profiles {
                    println!(
                        ...
                        style(&p.name).bold(),
                        p.hash_algo,
                        p.par2_redundancy.map(|v| v.to_string()).unwrap_or_else(|| "non".into()),
                        p.camera_mode,
                        p.auto_eject,
                    );
                }
...
fn cmd_history(action: HistoryAction) -> anyhow::Result<i32> {
    match action {
        ...
        HistoryAction::Show { id } => {
            match ferr_session::get_session(id)? {
        ...
        HistoryAction::Find { hash_or_name } => {
            ...
            if records.is_empty() {
                ...
                for r in &records {
                    println!(
                        "  session:{:5}  {}  {}",
                        r.session_id, r.path, style(&r.hash).dim()
                    );
                }
            }
        }
...
fn print_summary_table(
    ...
    let errors = manifest.files.iter()
    ...
    for dest in destinations {
        ...
        let t = if s.len() > 30 { format!("…{}", &s[s.len()-29..]) } else { s.into_owned() };
        println!(
            "  {:<30}  {:>10}  {:>10}  {:>10}  {:>7}  {}",
    ...
    if let Some(pct) = par2_pct {
        ...
        println!(
            "  {:<30}  {:>10}  {:>10}  {:>10}  {:>7}  {}",
            "PAR2", format!("{pct}%"), "(stub)", "-", "-", style("STUB").dim()
        );
    }
    ...
    println!(
        "  Total : {} fichiers · {} · {:.1}s",
        manifest.total_files, size_str, elapsed.as_secs_f64()
    );
}
